...
describe('the plane costs the untrusted side nothing to start', () => {
  ...
  for (const file of ['../src/plane.mjs', '../src/exec.mjs', '../src/fingerprint.mjs',
    // The TRUSTED side has no node_modules either: CI runs this file on the
    // runner before any container starts. Same rule, opposite side of the
    // boundary, and it is here because the manifest step silently could not run
    // for want of `yaml` until a review executed it.
