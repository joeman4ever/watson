...
describe('the plane costs the untrusted side nothing to start', () => {
  ...
  test('the whole import graph reachable from the plane is dependency-free', () => {
    // Walked transitively rather than checked file by file: what must not appear
    // anywhere in the closure is a third-party package, because the untrusted
    // side starts this from a read-only mount with nothing installed.
    const seen = new Set();
    const foreign = [];
    const visit = (url) => {
